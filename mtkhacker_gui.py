...
class DeviceHandler(QObject):
    ...
    def __init__(self, parent, preloader: str = None, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        config = Mtk_Config(gui=self.sendToLogSignal, guiprogress=self.sendToProgressSignal,
...
def getDevInfo(self, parameters):
    ...
    mtkClass = devhandler.download_agent.mtk
    download_agent = devhandler.download_agent
    try:
    ...
    mtkClass = download_agent.configure(mtkClass, preloader=None)
    if mtkClass:
...
class MainWindow(QMainWindow):
    ...
    def updateProgress(self, progress):
        try:
            if self.Status["rpmb"]:
                ...
                self.Status["currentPartitionSizeDone"] = progress * self.devhandler.download_agent.mtk.daloader.progress.pagesize
    ...
    def initread(self):
        self.readflash=ReadFlashWindow(self.ui, self, self.devhandler.download_agent, self.sendToLog)
    ...
    def initkeys(self):
        self.genkeys = generateKeysMenu(self.ui, self, self.devhandler.download_agent, self.sendToLog)
    ...
    def initunlock(self):
        self.unlock = UnlockMenu(self.ui, self, self.devhandler.download_agent, self.sendToLog)
    ...
    def initerase(self):
        self.eraseflash = EraseFlashWindow(self.ui, self, self.devhandler.download_agent, self.sendToLog)
    ...
    def initwrite(self):
        self.writeflash = WriteFlashWindow(self.ui, self, self.devhandler.download_agent, self.sendToLog)
    ...
    def getpartitions(self):
        data, guid_gpt = self.devhandler.download_agent.mtk.daloader.get_gpt()
        if guid_gpt is None:
    ...
    def updateGui(self):
        ...
        if phoneInfo['daInit']:
            ...
            self.pixmap = QPixmap(mtkhacker_path.images+"/phone_connected.png")
    ...
    def initpixmap(self):
        # phone spinner
        self.pixmap = QPixmap(mtkhacker_path.images+"/phone_loading.png").scaled(96, 96, Qt.KeepAspectRatio,
                                                                                Qt.SmoothTransformation)
        self.pixmap.setDevicePixelRatio(2)
        self.ui.spinner_pic.setPixmap(self.pixmap)
        self.ui.spinner_pic.show()

        nfpixmap = QPixmap(mtkhacker_path.images+"/phone_notfound.png")
        self.ui.pic.setPixmap(nfpixmap)

        initSteps = QPixmap(mtkhacker_path.images+"*/initsteps.png")
...
if __name__ == '__main__':
	...
	icon = QIcon()
	icon.addFile(mtkhacker_path.images+'/logo_32.png', QSize(32, 32))
	icon.addFile(mtkhacker_path.images+'/logo_64.png', QSize(64, 64))
	icon.addFile(mtkhacker_path.images+'/logo_256.png', QSize(256, 256))
	icon.addFile(mtkhacker_path.images+'/logo_512.png', QSize(512, 512))
	...
	if sys.platform.startswith('darwin'):  # MacOS has the toolbar in the top bar insted of in the app...
		addTopMargin = 0
	window_title = mtkhacker_meta.name + ' version ' + mtkhacker_meta.version
	win.setWindowTitle(window_title)
	
	win.show()
	
	# Device setup
	devhandler = DeviceHandler(parent=app, preloader=None)
	devhandler.sendToLogSignal.connect(win.sendToLog)
	# Get the device info
	thread = asyncThread(parent=app, n=0, function=getDevInfo, parameters=[phoneInfo, devhandler])
