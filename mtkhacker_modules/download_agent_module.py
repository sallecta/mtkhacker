...
class MTKHacker_Download_Agent:
		...
		try:
			...
			if idx != -1:
				...
				if idx != -1:
					...
					logger.info(f"Generated preloader file name is {filename}")
					if preloader is not None:
						if not os.path.exists(filename):
							try:
								with open(filename, "wb") as wf:
									wf.write(preloader)
									logger.info(f"Successfully extracted preloader for this device to: {filename}")
							except:
								pass
				return preloader
			logger.error('idx not found.')
		except Exception as err:
			logger.error(str(err))
			return None

	def configure(self, arg_mtk, arg_preloader):
		arg_mtk.connect.cdc.connected = arg_mtk.connect.cdc.connect()
		if arg_mtk.connect.cdc.connected is None or not arg_mtk.connect.cdc.connected:
			arg_mtk.preloader.init()
		else:
			if arg_mtk.connect.cdc.connected and os.path.exists(self.path.devicefile):
				info = arg_mtk.daloader.reinit()
				return arg_mtk
		if arg_mtk.config.target_config is None:
			logger.error("arg_mtk.config.target_config is None.")
			logger.info("Please disconnect, restart and connect.")
			return None
		if arg_mtk.config.target_config["daa"]:
			arg_mtk = arg_mtk.bypass_security()
			self.mtk = arg_mtk
			logger.info("Device is protected.")
			if arg_mtk is not None:
				if arg_mtk.config.is_brom:
					logger.info("Device is in BROM mode. Trying to dump preloader.")
					if arg_preloader is None:
						arg_preloader = self.dump_preloader_ram()
		else:
			logger.info("Device is unprotected.")
			# if not arg_mtk.config.is_brom:
			#   self.mtk.preloader.reset_to_brom()
			if arg_mtk.config.is_brom:
				logger.info("Device is in BROM mode. Bypassing security.")
				arg_mtk = arg_mtk.bypass_security()  # Needed for dumping preloader
				if arg_mtk is not None:
					self.mtk = arg_mtk
					if arg_preloader is None:
						logger.warning(
						...
						arg_preloader = self.dump_preloader_ram()
						if arg_preloader is None:
							logger.error("Failed to dump preloader from RAM.")
			else:
				logger.warning("Device is in Preloader mode :(")
		if not arg_mtk.daloader.upload_da(arg_preloader):
			return None
		else:
			arg_mtk.daloader.writestate()
			return arg_mtk
	...
	def da_read(self, partitionname, parttype, filename):
		...
		if parttype == "user" or parttype is None:
			i = 0
			countDump = 0
			logger.info("Requesting available partitions ....")
			...
			for partition in partitions:
				...
				if partition == "gpt":
					...
					if rpartition is not None:
						logger.info(f"Dumping partition \"{rpartition.name}\"")
						if self.mtk.daloader.readflash(addr=rpartition.sector * self.mtk.daloader.daconfig.pagesize,
													   ...
													   filename=partfilename, parttype=parttype):
							logger.info(f"Dumped sector {str(rpartition.sector)} with sector count " +
									  f"{str(rpartition.sectors)} as {partfilename}.")
							countDump += 1
						else:
							logger.info(f"Failed to dump sector {str(rpartition.sector)} with sector count " +
									  f"{str(rpartition.sectors)} as {partfilename}.")
							countDump += 1
					else:
						logger.error(f"Error: Couldn't detect partition: {partition}\nAvailable partitions:")
						for rpartition in gpttable:
							logger.info(rpartition.name)
			if countDump > 1 and countDump == len(filenames):
				logger.info(f"All partitions were dumped")
			elif countDump > 1 and countDump != len(filenames):
				logger.info(f"Failed to dump some partitions")
		else:
			i = 0
			for partfilename in filenames:
				...
				if self.mtk.daloader.readflash(addr=pos, length=0xFFFFFFFF, filename=partfilename,
											   parttype=parttype):
					logger.info(f"Dumped partition {str(partitionname)} as {partfilename}.")
				else:
					logger.info(f"Failed to dump partition {str(partitionname)} as {partfilename}.")
				i += 1
		...
		if guid_gpt is None:
			...
			for partition in guid_gpt.partentries:
				...
				if self.mtk.daloader.readflash(addr=partition.sector * self.mtk.daloader.daconfig.pagesize,
					...
					countGPT += 1
					logger.info(f"Dumped partition {str(partition.name)} as {str(filename)}.")
				else:
					countGPT -= 1
					logger.error(f"Failed to dump partition {str(partition.name)} as {str(filename)}.")

			partitionsForRead = len(guid_gpt.partentries) - len(skip)
			if countGPT == partitionsForRead:
				logger.info(f"All Dumped partitions success.")
			else:
				logger.error(f"Failed to dump all partitions")
	...
	def da_rf(self, filename, parttype):
		if self.mtk.daloader.daconfig.flashtype == "ufs":
			...
			length = self.mtk.daloader.daconfig.flashsize
		logger.info(f"Dumping sector 0 with flash size {hex(length)} as {filename}.")
		if self.mtk.daloader.readflash(0, length, filename, parttype):
			logger.info(f"Dumped sector 0 with flash size {hex(length)} as {filename}.")
		else:
			logger.info(f"Failed to dump sector 0 with flash size {hex(length)} as {filename}.")
		...
		if guid_gpt is None:
			...
			for partition in guid_gpt.partentries:
				if partition.name in pnames:
					logger.info(f"Detected partition: {partition.name}")
					...
					if (val & 0xFFFFFFF0) == 0xD0B5B1C0:
						with open(filename, "wb") as wf:
							wf.write(data)
							logger.info(f"Dumped footer from {partition.name} as {filename}.")
							return
		...
		if parttype == "user" or parttype is None:
			...
			for partfilename in filenames:
				...
				if self.mtk.daloader.writeflash(addr=pos, length=size, filename=partfilename,
												parttype=parttype):
					logger.info(f"Wrote {partfilename} to sector {str(pos // 0x200)} with " +
						  f"sector count {str(size)}.")
				else:
					logger.info(f"Failed to write {partfilename} to sector {str(pos // 0x200)} with " +
						  f"sector count {str(size)}.")
	...
	def da_wl(self, parttype: str, directory: str):
		...
		if parttype == "user" or parttype is None:
			...
			for partfilename in filenames:
				...
				i += 1
				if partition == "gpt":
					logger.info(f"Writing partition {partition}")
					if self.mtk.daloader.writeflash(addr=0,
													length=os.stat(partfilename).st_size,
													filename=partfilename,
													parttype=parttype):
						logger.info(f"Wrote {partition} to sector {str(0)}")
					else:
						logger.info(f"Failed to write {partition} to sector {str(0)}")
					continue
				res = self.mtk.daloader.detect_partition(partition, parttype)
				if res[0]:
					rpartition = res[1]
			...
			for partfilename in filenames:
				...
				partition = os.path.basename(partfilename)
				partition = os.path.splitext(partition)[0]
				logger.info(f"Writing filename {partfilename}")
				if self.mtk.daloader.writeflash(addr=pos, length=size, filename=partfilename,
	...
	def da_erase(self, partitions: list, parttype: str):
		...
		if countFP == len(partitions) and countFP > 1:
			logger.info(f"All partitions formatted.")
		elif countFP != len(partitions) and countFP > 1:
			logger.error(f"Failed to format all partitions.")
	...
	def da_ess(self, sector: int, sectors: int, parttype: str):
		if parttype == "user" or parttype is None:
			...
			self.mtk.daloader.formatflash(addr=sector * self.mtk.daloader.daconfig.pagesize,
										  ...
										  parttype=parttype,
										  display=True)
			logger.info(f"Formatted sector {str(pos // 0x200)}")
		...
		if parttype == "user" or parttype is None:
			...
			for partition in partitions:
				...
				if res[0]:
					...
					if sectors > rsectors:
						logger.error(f"Partition {partition} only has {rsectors}, you were using {sectors}. " +
								   f"Aborting")
						continue
					wipedata = b"\x00" * 0x200000
					error = False
	...
	def da_efuse(self):
		if self.mtk.config.chipconfig.efuse_addr is not None:
			...
			for idx in range(len(efuseconfig.efuses)):
				...
				logger.info(f"EFuse Idx {hex(idx)}: {data.hex()}")
	...
	def da_peek(self, addr: int, length: int, filename: str):
		...
		if filename is not None:
			wf.close()
			logger.info(f"Successfully wrote data from {hex(addr)}, length {hex(length)} to {filename}")
		else:
			logger.info(
	...
	def da_poke(self, addr: int, data: str, filename: str):
		...
		if self.mtk.daloader.poke(addr=addr, data=data):
			logger.info(f"Successfully wrote data to {hex(addr)}, length {hex(len(data))}")

	def handle_da_cmds(self, mtk, cmd: str, args):
		if mtk is None or mtk.daloader is None:
			logger.error("Error on running da, aborting :(")
			sys.exit(1)
		...
		if cmd == "gpt":
			...
			if not guid_gpt:
				logger.error("Error reading gpt, please read whole flash using \"mtk rf flash.bin\".")
			else:
				guid_gpt.print()
		elif cmd == "r":
			...
			parttype = args.parttype
			if filename is None:
				logger.error("No filename given to write to flash")
				self.close()
				return
			if not os.path.exists(filename):
				logger.error(f"Filename {filename} to write doesn't exist")
				self.close()
				return
			logger.info(f"Writing offset {hex(start)} with length {hex(length)}")
			...
			if args.sectors is None:
				logger.error("Sector count is missing. Usage: es [partname] [sector count]")
				self.close()
			partitions = partitionname.split(",")
			...
			sectors = getint(args.sectors)
			if args.sectors is None:
				logger.error("Sector count is missing. Usage: ess [sector] [sector count]")
				self.close()
			self.da_ess(sector=sector, parttype=parttype, sectors=sectors)
		elif cmd == "reset":
			if os.path.exists(self.path.devicefile):
				os.remove(self.path.devicefile)
				path_hwparam = (os.path.join(self.path.logs, "hwparam.json"))
				os.remove(path_hwparam)
