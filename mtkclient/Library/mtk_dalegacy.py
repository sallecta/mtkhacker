...
class DALegacy:
    ...
    def __init__(self, mtk, daconfig):
        self.emmc = None
        self.nand = None
        self.nor = None
        self.sdc = None
        ...
        self.usbwrite = self.mtk.connect.usbwrite
        self.usbread = self.mtk.connect.usbread
        self.echo = self.mtk.connect.echo
        self.rbyte = self.mtk.connect.rbyte
        self.rdword = self.mtk.connect.rdword
        self.rword = self.mtk.connect.rword
        self.sectorsize = self.daconfig.pagesize
        self.totalsectors = self.daconfig.flashsize
        self.partition = Partition(self.mtk, self.readflash, self.read_pmt)
        self.patch = False
        self.generatekeys = self.mtk.config.generatekeys
        if self.generatekeys:
            self.patch = True
        self.lft = legacyext(self.mtk, self)
        ...
        class GptEntries:
            ...
            def print(self):
                ...
                for partition in self.partentries:
                    logger.info("{:20} Offset 0x{:016x}, Length 0x{:016x}, Flags 0x{:08x}, UUID {}, Type {}".format(
    ...
    def get_part_info(self):
        res = self.mtk.connect.mtk_cmd(self.Cmd.SDMMC_READ_PMT_CMD, 1 + 4)  # 0xA5
    ...
    def check_security(self):
        ...
        ack = self.mtk.connect.mtk_cmd(cmd, 1)
        if ack == self.Rsp.ACK:
            return True
        return False
    ...
    def recheck(self):  # If Preloader is needed
        ...
        status = unpack(">I", self.mtk.connect.mtk_cmd(cmd, 1))[0]
        if status == 0x1799:
    ...
    def set_stage2_config(self, hwcode):
        ...
        logger.debug(f"hwcode is {hex(hwcode)}")
        ...
        if buffer == b'':
            logger.error("Didn't receive Stage2 dram info, please check usb cable/hub and retry.")
            return False
        logger.info("Received Stage2 dram info.")
        if toread == 4 and buffer == pack(">I", 0xBC3):
            ...
            pdram[1] = draminfo[:9]
            logger.info("DRAM config needed for : " + hexlify(draminfo).decode('utf-8'))
            if self.daconfig.emi is None:
                logger.info("Searching preloader...")
                found = False
                for root, dirs, files in os.walk(mtkhacker_path.preloaders):
                    for file in files:
                        with open(os.path.join(root, file), "rb") as rf:
                            data = rf.read()
                            if pdram[0] in data or pdram[1] in data:
                                preloader = os.path.join(root, file)
                                logger.info("Found preloader: " + file)
            ...
            if self.usbread(4) == pack(">I", 0xBC4):  # Nand_Status
                ...
                logger.info("Reading dram nand info ...")
                nand_ids = []
                ...
                if self.daconfig.emi is not None:  # toDo
                    ...
                    if ret == self.Rsp.NACK:
                        logger.error("EMI Config not accepted :(")
                        return False
                    if ret == self.Rsp.ACK:
                        logger.info("Sending dram info ...")
                        dramlength = len(self.daconfig.emi)
                        logger.debug(f"EMI version is {hex(self.daconfig.emiver)}")
                        if self.daconfig.emiver in [0xF, 0x10, 0x14, 0x15]:
                            dramlength = unpack(">I", self.usbread(0x4))[0]  # 0x000000BC
                            logger.info("RAM length: " + hex(dramlength))
                            logger.debug("Writing ACK to USB...")
                            self.usbwrite(self.Rsp.ACK)
                            ...
                            logger.info("RAM info: " + hexlify(info).decode('utf-8'))
                            ...
                            logger.warning("Unknown EMI version: %d" % self.daconfig.emiver)
                        ...
                        logger.debug(f"M_EXT_RAM_RET : {m_ext_ram_ret}")
                        if m_ext_ram_ret != 0:
                            logger.error("Preloader error: 0x%X => %s" % (m_ext_ram_ret, error_to_string(m_ext_ram_ret)))
                            self.mtk.connect.close(reset=False)
                            return False
                        m_ext_ram_type = self.usbread(1)[0]  # 0x02 HW_RAM_DRAM
                        logger.info(f"M_EXT_RAM_TYPE : {hex(m_ext_ram_type)}")
                        m_ext_ram_chip_select = self.usbread(1)[0]  # 0x00 CS_0
                        logger.info(f"M_EXT_RAM_CHIP_SELECT : {hex(m_ext_ram_chip_select)}")
                        m_ext_ram_size = unpack(">Q", self.usbread(8))[0]  # 0x80000000
                        logger.info(f"M_EXT_RAM_SIZE : {hex(m_ext_ram_size)}")
                    ...
                    logger.error("Preloader needed due to dram config. Closing connection.")
                    self.mtk.connect.close(reset=True)
                    return False
        return True
    ...
    def upload(self):
        if self.daconfig.da_loader is None:
            logger.error("No valid da loader found... aborting.")
            return False
        loader = self.daconfig.loader
        logger.info(f"Uploading legacy stage 1 from {os.path.basename(loader)}")
        with open(loader, 'rb') as bootldr:
            ...
            if self.mtk.preloader.send_da(da1address, da1size, da1sig_len, da1):
                if self.mtk.preloader.jump_da(da1address):
                    sync = self.usbread(1)
                    if sync != b"\xC0":
                        logger.error("Error on DA sync")
                        return False
                    else:
                        logger.info("Got loader sync!")
                else:
                    return False
            else:
                return False
            logger.info("Reading nand info")
            nandinfo = unpack(">I", self.usbread(4))[0]
            logger.debug("NAND_INFO: " + hex(nandinfo))
            ids = unpack(">H", self.usbread(2))[0]
            nandids = []
            for i in range(0, ids):
                tmp = unpack(">H", self.usbread(2))[0]
                nandids.append(tmp)
            logger.info("Reading emmc info")
            emmcinfolegacy = unpack(">I", self.usbread(4))[0]
            logger.debug("EMMC_INFO: " + hex(emmcinfolegacy))
            emmcids = []
            for i in range(0, 4):
            ...
            ackval += self.usbread(1)
            ackval += self.usbread(1)
            logger.debug("ACK: " + hexlify(ackval).decode('utf-8'))
            logger.info("Setting stage 2 config ...")
            if self.set_stage2_config(self.config.hwcode):
                logger.info("Uploading stage 2...")
                # stage 2
                if self.brom_send(self.daconfig, self.daconfig.da2, 2):
                    if self.read_flash_info():
                        ...
                        logger.info("Connected to preloader")
                        speed = self.check_usb_cmd()
                        if speed[0] == 0:  # 1 = USB High Speed, 2= USB Ultra high speed
                            logger.info("Reconnecting to preloader")
                            self.set_usb_cmd()
                            self.mtk.connect.close(reset=False)
                            time.sleep(1)
                            while not self.mtk.connect.cdc.connect():
                                logger.info("Waiting for reconnection")
                                time.sleep(0.5)
                            if self.check_usb_cmd():
                                logger.info("Connected to preloader")
                                self.mtk.connect.cdc.set_fast_mode(True)
                            else:
                                return False
                        return True
        return False
    ...
    def upload_da(self):
        logger.info("Uploading legacy da...")
        if self.upload():
            logger.info(self.flashconfig)
    ...
    def brom_send(self, dasetup, dadata, stage, packetsize=0x1000):
        ...
        if buffer == self.Rsp.ACK:
            ...
            buffer = self.usbread(1)
            if buffer == self.Rsp.ACK:
                logger.info(f"Successfully uploaded STAGE {stage}")
                self.config.set_gui_status(self.config.tr(f"Successfully uploaded stage {stage}"))
                return True
        else:
            logger.error(f"Error on sending BROM stage {stage} : " + hexlify(buffer).decode('utf-8'))
            self.config.set_gui_status(self.config.tr("Error on sending BROM stage"))
        return False
    ...
    def sdmmc_write_data(self, addr, length, filename, offset=0, parttype=None, wdata=None, display=True):
        ...
        if self.usbread(1) != self.Rsp.ACK:
            logger.error("Couldn't send sdmmc_write_data header")
            return False
        offset = 0
        while offset < length:
            self.usbwrite(self.Rsp.ACK)
            ...
            if self.usbread(1) != self.Rsp.CONT_CHAR:
                logger.error("Data ack failed for sdmmc_write_data")
                return False
        ...
        if self.daconfig.flashtype == "emmc":
            ...
            while progress != 100:
                ...
                if ack is not self.Rsp.ACK[0]:
                    logger.error(f"Error on sending emmc read command, response: {hex(ack)}")
                    exit(1)
                ack = self.usbread(1)[0]
                if ack is not self.Rsp.ACK[0]:
                    logger.error(f"Error on sending emmc read command, response: {hex(ack)}")
    ...
    def readflash(self, addr, length, filename, parttype=None, display=True):
        ...
        if self.daconfig.flashtype == "emmc":
            ...
            if ack is not self.Rsp.ACK[0]:
                ...
                logger.error(f"Error on sending emmc read command, response: {hex(ack)}, status: {hex(res)}")
                exit(1)
            ...
            if ack is not self.Rsp.ACK:
                logger.error(f"Error on sending nand read command, response: {hex(ack)}")
                exit(1)
