...
class DAloader:
    def __init__(self, mtk):
        ...
        self.daconfig = DAconfig(mtk=self.mtk, loader=self.mtk.config.loader,
                                 preloader=self.mtk.config.preloader)
        self.hwparam = MTKHhacker_hwparam(mtk.config.meid)
    ...
    def reinit(self):
        if os.path.exists(self.path.devicefile):
            ...
            if self.xflash:
                self.da = DAXFlash(self.mtk, self.daconfig)
                self.daconfig.flashtype = config["flashtype"]
                self.daconfig.flashsize = config["flashsize"]
                self.da.reinit()
                self.xft = xflashext(self.mtk, self.da)
                self.lft = None
            else:
                self.da = DALegacy(self.mtk, self.daconfig)
                self.daconfig.flashtype = config["flashtype"]
                self.daconfig.flashsize = config["flashsize"]
                self.da.nor = norinfo()
                ...
                self.lft = legacyext(self.mtk, self.da)
