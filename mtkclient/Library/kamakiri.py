...
class Kamakiri():
    ...
    def exploit(self, payload, payloadaddr):
        ...
        self.mtk.connect.echo(b"\xE0")
        self.mtk.connect.echo(pack(">I", len(payload)))
        status = int.from_bytes(self.mtk.connect.usbread(2), byteorder="little")
        if status:
            raise Exception("Kamakiri Payload is too large")
        self.mtk.connect.usbwrite(payload)
        self.mtk.connect.usbread(2)
        self.mtk.connect.usbread(2)
    ...
    def kamakiri2(self, addr):
        self.udev = self.mtk.connect.cdc.device
        try:
    ...
    def exploit2(self, payload, payloadaddr=None):
        ...
        try:
            # self.mtk.connect.cdc.device.reset()
            if self.linecode is None:
                self.linecode = self.mtk.connect.cdc.device.ctrl_transfer(0xA1, 0x21, 0, 0, 7) + array.array('B', [0])
    ...
    def payload(self, payload, addr, forcekamakiri=True, exploittype=1):
        if self.mtk.config.target_config["sla"] or self.mtk.config.target_config["daa"] or forcekamakiri:
            ...
            if exploittype == 1:
                logger.info("Trying kamakiri..")
                if self.exploit(payload, addr):
                    logger.info("Done sending payload...")
                    return True
            elif exploittype == 2:
                logger.info("Trying kamakiri2..")
                if self.exploit2(payload, addr):
                    logger.info("Done sending payload...")
                    time.sleep(0.2)
                    return True
        else:
            logger.info("Sending payload via insecure da.")
            ...
            if self.mtk.preloader.send_da(addr, len(payload) - 0x100, 0x100, payload):
                if self.mtk.preloader.jump_da(addr):
                    logger.info("Done sending payload...")
                    return True
        logger.error("Error on sending payload.")
        return False
    ...
    def bruteforce2(self, args, startaddr=0x9900):
        ...
        while not found:
            ...
            if self.mtk.preloader.init(display=False):
                ...
                logger.info("Bruteforce, testing " + hex(startaddr) + "...")
                if self.linecode is None:
                    self.linecode = self.mtk.connect.cdc.device.ctrl_transfer(0xA1, 0x21, 0, 0, 7) + array.array('B', [0])
                ...
                if found:
                    ...
                    logger.info("Found " + hex(startaddr) + f", dumping bootrom to {filename}")
    ...
    def newbrute(self, dump_ptr, dump=False):
        ...
        if dump:
            ...
            for address in range(dump_ptr, 0xffff, 4):
                if address % 0x100 == 0:
                    logger.info("Bruteforce, testing " + hex(address) + "...")
                for i in range(3):
    ...
    def bruteforce(self, args):
        ...
        filename = os.path.join(self.path.payloads, "generic_dump_payload.bin")
        try:
            with open(filename, "rb") as rf:
                payload = rf.read()
                logger.info(f"Loading payload from {filename}, {hex(len(payload))} bytes")
        ...
        if var1 == 0xFF:
            logger.error("Couldn't find the right var1 value.")
        self.close()
        return False
    ...
    def dump_brom(self, filename):
        try:
            with open(filename, 'wb') as wf:
                ...
                length = self.mtk.connect.usbread(4)
                length = int.from_bytes(length, 'big')
                rlen = min(length, 0x20000)
                for i in range(length // rlen):
                    data = self.mtk.connect.usbread(rlen)
                    wf.write(data)
                    print_progress(i, length // rlen, prefix='Progress:', suffix='Complete', bar_length=50)
                print_progress(100, 100, prefix='Progress:', suffix='Complete', bar_length=50)
                return True
        except Exception as e:
            logger.error(f"Error on opening {filename} for writing: {str(e)}")
            return False

    def dump_preloader(self):
        try:
            filename = ""
            length = unpack("<I", self.mtk.connect.usbread(4))[0]
            if length > 0:
