...
class sej:
    ...
    def SEJ_Init_MTEE(self, encrypt=True, iv=None):
        ...
        for val in [[0x2d44bb70,0xa744d227,0xd0a9864b,0x83ffc244],
            ...
            while i < 20:
                ...
                i += 1
            if i == 20:
                logger.error("SEJ Hardware seems not to be configured correctly. Results may be wrong.")
    ...
    def hw_aes128_cbc_encrypt(self, buf, encrypt=True, iv=None):
        if iv is None:
            iv = self.g_HACC_CFG_1
        self.tz_pre_init()
        logger.info("HACC init")
        self.SEJ_V3_Init(ben=encrypt, iv=iv)
        logger.info("HACC run")
        buf2 = self.SEJ_Run(buf)
        logger.info("HACC terminate")
        self.SEJ_Terminate()
        return buf2
