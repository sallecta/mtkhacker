...
class PLTools:
    def __init__(self, mtk):
        ...
        self.hwcrypto = hwcrypto(setup, self.mtk.config.gui)
    ...
    def runpayload(self, filename, offset=0, ack=0xA1A2A3A4, addr=None, dontack=False):
        ...
        try:
            with open(filename, "rb") as rf:
                rf.seek(offset)
                payload = rf.read()
                logger.info(f"Loading payload from {os.path.basename(filename)}, {hex(len(payload))} bytes")
        except FileNotFoundError:
            logger.info(f"Couldn't open {filename} for reading.")
            return False
        ...
        if ptype == "amonet":
            logger.info("Amonet Run")
            if self.payload(payload, addr, ptype):
                if dontack:
                    return True
                result = self.usbread(4)
                if result == pack(">I", ack):
                    logger.info("Successfully sent payload: " + filename)
                    return True
                logger.info("Error, payload answered instead: " + hexlify(result).decode('utf-8'))
                return False
            else:
                self.error("Error on sending payload: " + filename)
            return True
        elif ptype in ("kamakiri", "kamakiri2"):
            logger.info("Kamakiri / DA Run")
            ...
            logger.info("Carbonara is best served at local restaurants.")
        else:
            logger.info("Kamakiri / DA Run")
            if self.kama.payload(payload, addr, False, exploittype=2):
                ...
                if result == b"\xc1\xc2\xc3\xc4":
                    if "preloader" in rf.name:
                        ack = self.mtk.connect.usbread(4)
                        if ack == b"\xC0\xC0\xC0\xC0":
                            with open("preloader.bin", 'wb') as wf:
                                print_progress(0, 100, prefix='Progress:', suffix='Complete', bar_length=50)
                                for pos in range(0, 0x40000, 64):
                                    wf.write(self.mtk.connect.usbread(64))
                                logger.info("Preloader dumped as: " + "preloader.bin")
                                return True
                    else:
                        with open("out.bin", 'wb') as wf:
                            ...
                            for pos in range(0, 0x20000, 64):
                                wf.write(self.mtk.connect.usbread(64))
                            logger.info("Bootrom dumped as: " + "out.bin")
                            return True
                logger.info("Error, payload answered instead: " + hexlify(result).decode('utf-8'))
                return False
            else:
    ...
    def crasher(self, mtk, enforcecrash: bool = False):
        ...
        if enforcecrash or self.config.meid is None:
            logger.info("We're not in bootrom, trying to crash da...")
    ...
    def run_dump_brom(self, filename, btype, loader="generic_dump_payload.bin"):
        ...
        if loader == "generic_sram_payload.bin":
            length = 0x200000
        pfilename = os.path.join(self.path.payloads, loader)
        if btype == "amonet":
            if self.dump_brom(filename, "gcpu", length=length):
                logger.info("Dumped as: " + filename)
                return True
            else:
            ...
            logger.info("Kamakiri2")
            if self.mtk.config.chipconfig.send_ptr[0] is None:
                logger.info("Unknown chipset, please run \"brute\" command and send the brom as an issue on github")
                return False
            if self.runpayload(filename=pfilename, ack=0xC1C2C3C4, offset=0):
                if self.kama.dump_brom(filename):
                    logger.info("Dumped as: " + filename)
            ...
            logger.info("Available ptypes are: amonet, kamakiri, kamakiri2, hashimoto, carbonara")
        self.error("Error on dumping.")
        return False

    def run_dump_preloader(self, btype):
        pfilename = os.path.join(self.path.payloads, "generic_preloader_dump_payload.bin")
        """
        if btype == "amonet":
            if self.dump_brom(filename, "gcpu"):
                logger.info("Preloader dumped as: " + filename)
                return True
            else:
                self.error("Error on sending payload: " + pfilename)
        elif btype == "hashimoto":
