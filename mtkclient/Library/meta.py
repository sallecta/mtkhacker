...
class META:
    ...
    def init(self, arg_metamode: bytes, arg_maxtries=None, arg_display=True):
        if not arg_display:
        ...
        while not cdc.connected:
            try:
                if arg_maxtries is not None and counter == arg_maxtries:
                    break
                ...
                if cdc.connected and cdc.pid == 0x2000:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            break
                        if resp==b"READY":
                            EP_OUT(arg_metamode, len(arg_metamode))
                            while resp==b"READY":
                                resp = bytearray(EP_IN(maxinsize))
