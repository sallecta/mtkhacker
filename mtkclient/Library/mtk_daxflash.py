...
class DAXFlash:
    ...
    def __init__(self, mtk, daconfig):
        ...
        if self.generatekeys:
            self.patch = True
        self.xft = xflashext(self.mtk, self)
    ...
    def reinit(self, display=False):
        self.config.hwparam = MTKHhacker_hwparam(self.config.meid, self.config.hwparam_path)
        ...
        if self.upload():
            ...
            if connagent == b"brom":
                ...
                if self.daconfig.emi is None:
                    ...
                    for root, dirs, files in os.walk(os.path.join(self.path.loaders, 'Preloader')):
                        for file in files:
