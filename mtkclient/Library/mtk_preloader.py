...
class Preloader:
	...
	def init(self, maxtries=None, display=True):
		...
		logger.info("Waiting for PreLoader VCOM, please reconnect mobile to BROM mode")
		self.config.set_gui_status(self.config.tr("Waiting for PreLoader VCOM"))
		
		success = False
		tries = 0
		while not success and tries < 10:
			success = self.mtk.connect.handshake(maxtries=maxtries)
			if not success:
				logger.error("Handshake failed, retrying...")
				self.config.set_gui_status(self.config.tr("Status: Handshake failed, retrying..."))
				self.mtk.connect.close()
				tries += 1
		if tries == 10:
			return False
		...
		if self.display:
			logger.info("\tCPU:\t\t\t" + cpu + "(" + self.config.chipconfig.description + ")")
			self.config.cpu = cpu.replace("/", "_")
			logger.info("\tHW version:\t\t" + hex(self.config.hwver))
			logger.info("\tWDT:\t\t\t" + hex(self.config.chipconfig.watchdog))
			logger.info("\tUart:\t\t\t" + hex(self.config.chipconfig.uart))
			logger.info("\tBrom payload addr:\t" + hex(self.config.chipconfig.brom_payload_addr))
			logger.info("\tDA payload addr:\t" + hex(self.config.chipconfig.da_payload_addr))
			if self.config.chipconfig.cqdma_base is not None:
				logger.info("\tCQ_DMA addr:\t\t" + hex(self.config.chipconfig.cqdma_base))
			logger.info("\tVar1:\t\t\t" + hex(self.config.chipconfig.var1))
		...
		if self.display:
			logger.info("HW code:\t\t\t" + hex(self.config.hwcode))
		self.config.target_config = self.get_target_config(self.display)
		logger.info("Get Target info")
		self.get_blver()
		self.get_bromver()
		...
		if self.display:
			logger.info("\tHW subcode:\t\t" + hex(self.config.hwsubcode))
			logger.info("\tHW Ver:\t\t\t" + hex(self.config.hwver))
			logger.info("\tSW Ver:\t\t\t" + hex(self.config.swver))
		meid = self.get_meid()
		if meid is not None:
			self.config.set_meid(meid)
			if self.display:
				logger.info("ME_ID:\t\t\t" + hexlify(meid).decode('utf-8').upper())
			if readsocid or self.config.chipconfig.socid_addr:
				...
				if self.display:
					if socid != b"":
						logger.info("SOC_ID:\t\t\t" + hexlify(socid).decode('utf-8').upper())
		return True
	def __checksum_xflash(arg_data):
		checksum = 0
		pos = 0
		for i in range(0, len(arg_data) // 4):
			checksum += unpack("<I", arg_data[i * 4:(i * 4) + 4])[0]
			pos += 4
		if len(arg_data) % 4 != 0:
			for i in range(4 - (len(arg_data) % 4)):
				checksum += arg_data[pos]
				pos += 1
	...
	def send_partition_data(self, partitionname, data):
		checksum = __checksum_xflash(data)
		if isinstance(partitionname, str):
			partitionname = bytes(partitionname, 'utf-8')[:64]
		partitionname = partitionname + (64 - len(partitionname)) * b'\x00'
	...
	def get_blver(self):
		if self.usbwrite(self.Cmd.GET_BL_VER.value):
			...
			if res == self.Cmd.GET_BL_VER.value:
				# We are in boot rom ...
				logger.info("BROM mode detected.")
	...
	def get_target_config(self, display=True):
		if self.echo(self.Cmd.GET_TARGET_CONFIG.value):
			...
			if display:
				logger.info(f"Target config:\t\t{hex(target_config)}")
				logger.info(f"\tSBC enabled:\t\t{sbc}")
				logger.info(f"\tSLA enabled:\t\t{sla}")
				logger.info(f"\tDAA enabled:\t\t{daa}")
				logger.info(f"\tSWJTAG enabled:\t\t{swjtag}")
				logger.info(f"\tEPP_PARAM at 0x600 after EMMC_BOOT/SDMMC_BOOT:\t{epp}")
				logger.info(f"\tRoot cert required:\t{cert}")
				logger.info(f"\tMem read auth:\t\t{memread}")
				logger.info(f"\tMem write auth:\t\t{memwrite}")
				logger.info(f"\tCmd 0xC8 blocked:\t{cmd_c8}")
	...
	def jump_da(self, addr):
		logger.info(f"Jumping to {hex(addr)}")
		...
		if self.echo(self.Cmd.JUMP_DA.value):
			...
			if resaddr == addr:
				try:
					...
					return False
				if status == 0:
					logger.info(f"Jumping to {hex(addr)}: ok.")
	...
	def brom_register_access(self, address, length, data=None, check_status=True):
		if data is None:
			...
			mode = 1
		if self.mtk.connect.echo(self.Cmd.brom_register_access.value):
			self.mtk.connect.echo(pack(">I", mode))
			self.mtk.connect.echo(pack(">I", address))
			self.mtk.connect.echo(pack(">I", length))
			status = self.mtk.connect.usbread(2)
			try:
			...
			if mode == 0:
				data = self.mtk.connect.usbread(length)
			else:
				self.mtk.connect.usbwrite(data[:length])
