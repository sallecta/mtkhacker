...
class MTKH_connect_USB:
			...
			if data[:5] == b"<?xml":
				try:
					...
					for line in data.split(b"\n"):
						try:
							logger.debug(pre + line.decode('utf-8'))
							rdata += line + b"\n"
						except:
							v = hexlify(line)
					...
					pass
		# logger.debug(pre + hexlify(data).decode('utf-8'))
		return data

	def getInterfaceCount(self):
		if self.vid is not None:
			self.device = usb.core.find(self.vid, self.pid, self.backend)
			if self.device is None:
				logger.error("Couldn't detect the device. Is it connected?")
				return False
			try:
			...
			logger.debug(f"self.configuration is {self.configuration}")
	...
	def connect(self, EP_IN=-1, EP_OUT=-1):
		...
		# logger.debug(f"self.portconfig{self.portconfig}")
		for dev in devices:
		...
		if self.device is None:
			#logger.warning("Couldn't detect the device, is it connected?")
			return False
		logger.info("Device detected, getting configuration...")
		try:
			self.configuration = self.device.get_active_configuration()
		...
		if self.configuration is None:
			...
			return False
		logger.info("Got device configuration. Getting interface...")
		if self.interface == -1:
		...
		if command == b'':
			...
			while pos < len(command):
				try:
					...
					logger.error(f"Error while writing to USB: {str(err)}")
					# time.sleep(0.01)
					i += 1
					if i == 3:
						return False
	...
	def usbread(self, resplen=None, maxtimeout=100):
		...
		if resplen <= 0:
			logger.info(f"resplen <= 0{str(resplen)}")
		res = bytearray()
		timeout = 0
		epr = self.EP_IN.read
		...
		while len(res) < resplen:
			try:
				...
				if "timed out" in error:
					...
					return b""
				elif "Input/Output" in error:
					logger.error("USB Input/Output error")
					return b""
				else:
					logger.error(f"USB error {repr(e)}")
					return b""

		logger.debug(inspect.currentframe().f_back.f_code.co_name + ":" + hex(resplen))
...
class Scsi:
	...
	def __init__(self, vid=None, pid=None, interface=-1):
		self.vid = vid
		self.pid = pid
	...
	def connect(self):
		self.usb = MTKH_connect_USB(portconfig=[self.vid, self.pid, self.interface], devclass=8)
		if self.usb.connect():
	...
	def send_mass_storage_command(self, lun, cdb, direction, data_length):
		...
		if len(cdb) != cdb_len:
			logger.error("cdb length doesn't fit allowed cbw packet length")
			return 0

		if (cdb_len == 0) or (cdb_len > command_block_wrapper_len):
			logger.error(f"Data packet length should be max of {str(command_block_wrapper_len)} bytes, {str(cdb_len)} given.")
			return 0
		else:
			data = write_object(command_block_wrapper, b"USBC", tag, data_length, direction, lun, cdb_len, cdb)[
				'raw_data']
			if len(data) != 31:
				logger.error("Invalid data packet length, should be 31 bytes, but length is %d" % len(data))
				return 0
			tag += 1
	...
	def send_htc_adbenable(self):
		...
		logger.info("Sending HTC adb enable command")
		...
		if datasize > 0:
			...
			logger.debug("DATA: " + hexlify(data).decode('utf-8'))
		logger.info("Sent HTC adb enable command")

	def send_htc_ums_adbenable(self):  # HTC10
		# ums_ctrlrequest from f_mass_storage.c
		logger.info("Sending HTC ums adb enable command")
		...
		logger.info("Sent HTC ums adb enable command: %x" % ret)
	...
	def send_zte_adbenable(self):  # zte blade
		...
		if datasize > 0:
			data = self.usb.read(datasize, timeout)
			logger.info("DATA: " + hexlify(data).decode('utf-8'))
		logger.info("Send HTC adb enable command")

	def send_fih_adbenable(self):  # motorola xt560, nokia 3.1, #f_mass_storage.c
		if self.usb.connect():
			logger.info("Sending FIH adb enable command")
			datasize = 0x24
			...
			if datasize > 0:
				data = self.usb.read(datasize, timeout)
				logger.info("DATA: " + hexlify(data).decode('utf-8'))
			logger.info("Sent FIH adb enable command")
			self.usb.close()

	def send_alcatel_adbenable(self):  # Alcatel MW41
		if self.usb.connect():
			logger.info("Sending alcatel adb enable command")
			datasize = 0x24
			...
			if datasize > 0:
				data = self.usb.read(datasize, timeout)
				logger.info("DATA: " + hexlify(data).decode('utf-8'))
			logger.info("Sent alcatel adb enable command")
			self.usb.close()
	...
	def send_fih_root(self):
		...
		if self.usb.connect():
			logger.info("Sending FIH root command")
			datasize = 0x24
			...
			if datasize > 0:
				data = self.usb.read(datasize, timeout)
				logger.info("DATA: " + hexlify(data).decode('utf-8'))
			logger.info("Sent FIH root command")
			self.usb.close()
